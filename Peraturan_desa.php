  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      body {
        ...
        <!-- Page Header-->
        <header class="masthead" style="background-image: url('assets/img/bendera-bg.jpg')">
        ...
        <main class="mb-4">
            <div class="container px-4 px-lg-5">
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    ...
                    <?php
include "koneksi.php";
$query = "SELECT * FROM peraturan_desa ORDER BY id_peraturan DESC";
$result = mysqli_query($conn, $query);

if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<div class='post-preview'>";
        echo "<div class='row mb-8'>
                <div class='col-12'>
                    <div class='card'>
                        <div class='card-body px-6 pb-6 d-flex'>
                            <div class='w-65px flex-shrink-0 d-none d-sm-block'>
                                <i class='bi bi-file-earmark-text-fill fs-3x'></i>
                            </div>
                            <div class='flex-grow-1'>
                                <div class='row g-4 g-xl-9 mb-4'>
                                    <div class='col-lg-8 fw-bold fs-4'>";
        echo "<h2><a href='peraturan.php?id=" . $row['id_peraturan'] . "'>" . htmlspecialchars($row['nomor_peraturan']) . "</a></h2>";
        echo "</div>";
        echo "<div class='col-lg-10'>
                <ol type='a' class='opacity-75'>
                    <li class='text-left mb-2'>
                        <div>
                            <ul class='list-inline'>
                                <li class='list-inline-item fw-bold'>Download :</li>
                                <li class='list-inline-item'><a class='text-danger' href='Admin/controlers/download.php?file=" . urlencode($row['isi_peraturan']) . "'><i class='bi bi-file-earmark-pdf me-2'></i>" .  htmlspecialchars($row['judul_peraturan']) . "</a></li>
                            </ul>
                        </div>
                    </li>
                </ol>";
        echo "<p class='post-meta'> Tahun : " . htmlspecialchars($row['penetapan']) . "</p>";
        echo "</div></div></div></div></div>";
        echo "</div>"; // Closing tags added
    }
} else {
