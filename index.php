...
<div class="container px-4 px-lg-5">
    <div class="row gx-4 gx-lg-5 justify-content-center">
        <div class="col-md-10 col-lg-8 col-xl-7">
            ...
            <?php
            include "koneksi.php";
            $query = "SELECT * FROM postingan ORDER BY tanggal DESC";
            $result = mysqli_query($conn, $query);

            if (mysqli_num_rows($result) > 0) {
                while($row = mysqli_fetch_assoc($result)) {
                    echo "<div class='post-preview'>";
                    echo "<a href='blog.php?id=" . $row['id_postingan'] . "'>";
                    echo "<h2 class='post-title'>" . htmlspecialchars($row['judul']) . "</h2>";
                    // Display image if available
                    if ($row['gambar']) {
                        echo "<img src='Admin/" . htmlspecialchars($row['gambar']) . "' alt='" . htmlspecialchars($row['judul']) . "' style='width:100%; height:auto;'>";
                    }
                    echo "</a>";
                    echo "<p class='post-meta'>Posted on: " . htmlspecialchars($row['tanggal']) . "</p>";
                    echo "</div>";
                }
            } else {
            ...
            <div class="d-flex justify-content-end mb-4"><a class="btn btn-primary text-uppercase" href="index.php"> New Post→</a></div>
        </div>
    </div>
</div>
